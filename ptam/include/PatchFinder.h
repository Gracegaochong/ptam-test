...
public:
    ...
    // Step 3 Functions  当前帧   与源帧对比计算零均值SSD得分
...
protected:
    ...
    int mnPatchSize; // Size of one side of the matching template.  8
    ...
    CVD::Image<CVD::byte> mimTemplate;   // The matching template  源图像8*8模板
    CVD::Image<std::pair<float,float> > mimJacs;  // Inverse composition jacobians; stored as floats to save a bit of space.

    Matrix<2> mm2WarpInverse;   // Warping matrix 变形矩阵的逆矩阵
    int mnSearchLevel;          // Search level in input pyramid 搜索层级
    ...
    CVD::ImageRef mirCenter;    // Quantized location of the center pixel of the NxN pixel template  （4,4）
