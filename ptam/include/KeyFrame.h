...
struct Candidate
{
    CVD::ImageRef irLevelPos;//角点坐标
    Vector<2> v2RootPos;//第0层位置
    double dSTScore;
};
...
struct Measurement
{
    int nLevel;   // Which image level?  层数
    bool bSubPix; // Has this measurement been refined to sub-pixel level? 第1和第2帧的图像均为true
    Vector<2> v2RootPos;  // Position of the measurement, REFERED TO PYRAMID LEVEL ZERO 角点第0层位置
