...
//    exit(1);
    ...
    //cout<<"Tracker::TrackFrame  mbUseSBIInit="<<mbUseSBIInit<<endl;
    //cout<<"Tracker::TrackFrame SmallBlurryImage *gvdSBIBlur="<<*gvdSBIBlur<<endl;
    if(!mpSBIThisFrame)
    {
    ...
    //
    cout<<"Tracker::TrackFrame:00"<<mMap.IsGood()<<endl;
    ...
    if(mMap.IsGood())
    {
        ...
        {
            if(mbUseSBIInit)
                CalcSBIRotation();//SE2---SE3  mv6SBIRot
            ApplyMotionModel();       //
            cout << "ApplyMotionModel end:" << endl;
            cout << mse3CamFromWorld << endl;
            TrackMap();               //  These three lines do the main tracking work.
            cout << "TrackMap() end:" << endl;
            cout << mse3CamFromWorld << endl;
            UpdateMotionModel();      //
cout<<"fame id:"<<mnFrame<<"  se3:"<< mse3CamFromWorld<<endl;
        ...
        Vector<3,double> translation = mse3CamFromWorld.get_translation();
        cout << "Tracker::TrackFrame(mMap.IsGood): se3TrackerPose translation:" << translation << endl;
    }
...
void Tracker::TrackForInitialMap()//��ʼ����ͼ
{
    cout<<"Tracker::TrackForInitialMap enter......"<<endl;
    ...
    // What stage of initial tracking are we at? 3��״̬
    if(mnInitialStage == TRAIL_TRACKING_NOT_STARTED)   //11111
    {
    ...
    if(mnInitialStage == TRAIL_TRACKING_STARTED)//2222222
    {
...
//avPVS
    ...
    for(unsigned int i=0; i<mMap.vpPoints.size(); i++)
    {
        ...
        TData.Project(mse3CamFromWorld, mCamera);//将世界点投影至像素点，判断是否在图像上
        if(!TData.bInImage)
            continue;

        // Calculate camera projection derivatives of this point.
        TData.GetDerivsUnsafe(mCamera);//J1

        // And check what the PatchFinder (included in TrackerData) makes of the mappoint in this view..
        TData.nSearchLevel = TData.Finder.CalcSearchLevelAndWarpMatrix(TData.Point, mse3CamFromWorld, TData.m2CamDerivs);//   1111
        //确定搜索金字塔层数
        ...
        TData.bFound = false;
        avPVS[TData.nSearchLevel].push_back(&TData);//分层保存点  得到avPVS集合
    };
    ...
    for(int i=0; i<LEVELS; i++)
        random_shuffle(avPVS[i].begin(), avPVS[i].end());//随机排序
    ...
    // Set of heuristics to check if we should do a coarse tracking stage.启发式
...
//vNextToSearch
    ...
    // with preference to LEVELS-1.  第3、4层
    if(bTryCoarse && avPVS[LEVELS-1].size() + avPVS[LEVELS-2].size() > *gvnCoarseMin )//20
    {
        ...
        if(avPVS[LEVELS-1].size() <= nCoarseMax)//第4层  点数少，全部取出来；点数多，随机删除
        ...
        //第4层点数不够，从第3层加点；  3\4层之和小于nCoarseMax，将第三层的全部取出来；大于nCoarseMax，随机删除
...
//粗追踪计算   10次高斯牛顿更新位姿
        ...
        unsigned int nFound = SearchForPoints(vNextToSearch, nCoarseRange, *gvnCoarseSubPixIts);// 30  8
                ...
                { // Re-project the points on all but the first iteration.
                    for(unsigned int i=0; i<vIterationSet.size(); i++)
                        if(vIterationSet[i]->bFound)
                            vIterationSet[i]->ProjectAndDerivs(mse3CamFromWorld, mCamera);//J1
                }
                for(unsigned int i=0; i<vIterationSet.size(); i++)
                    if(vIterationSet[i]->bFound)
                        vIterationSet[i]->CalcJacobian();//J123
...
//精跟踪阶段 fine tracking stage  点数多
    ...
    // so do all of these, with sub-pixel refinement.高层的点特征较好
    {
        int l = LEVELS - 1;
    ...
    int nFinePatchesToUse = *gvnMaxPatchesPerFrame - vIterationSet.size();//使用1000个点
    ...
    cout<<"mCurrentKF.dSceneDepthMean:"<<mCurrentKF.dSceneDepthMean<<endl;
    cout<<"mCurrentKF.dSceneDepthSigma:"<<mCurrentKF.dSceneDepthSigma<<endl;
    //exit(1);
}

// Find points in the image. Uses the PatchFiner struct stored in TrackerData
int Tracker::SearchForPoints(vector<TrackerData*> &vTD, int nRange, int nSubPixIts)// 30  8
{// 30  8
    int nFound = 0;
    for(unsigned int i=0; i<vTD.size(); i++)   // for each point..对每一个角点
    {
        ...
        Finder.MakeTemplateCoarseCont(TD.Point);//源帧8×8模板      2222222
        ...
        bool bFound =
                Finder.FindPatchCoarse(ir(TD.v2Image), mCurrentKF, nRange);//像素点 当前帧 30 得到匹配点    3333
        TD.bSearched = true;
        if(!bFound)
        {
            TD.bFound = false;
        ...
        // Found the patch in coarse search - are Sub-pixel iterations wanted too?  粗计算得到匹配的整数像素点，想要得到更好的亚像素点
        if(nSubPixIts > 0)//8
        {
            TD.bDidSubPix = true;
            Finder.MakeSubPixTemplate();//mm3HInv  //对源 模板图的每一点求灰度的梯度    444444
            bool bSubPixConverges=Finder.IterateSubPixToConvergence(mCurrentKF, nSubPixIts);//8  获得亚像素    55555
            if(!bSubPixConverges)
...
void Tracker::ApplyMotionModel()//mv6SBIRot
{
...
void Tracker::UpdateMotionModel()//mse3CamFromWorld
{
    ...
    mv6CameraVelocity = 0.9 * (0.5 * v6Motion + 0.5 * v6OldVel);//mv6CameraVelocity
...
void Tracker::CalcSBIRotation()//SE2---SE3
{
    mpSBILastFrame->MakeJacs();//上一帧图像 模板图的梯度值
    ...
    mv6SBIRot = se3Adjust.ln();//Vector<6> mv6SBIRot 只是得到旋转 还没有得到平移量
}
