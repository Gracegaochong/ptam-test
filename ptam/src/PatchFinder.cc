...
PatchFinder::PatchFinder(int nPatchSize)//8×8模板  nPatchSize=8
    : mimTemplate(ImageRef(nPatchSize,nPatchSize))
{
    mnPatchSize = nPatchSize;
    mirCenter = ImageRef(nPatchSize/2, nPatchSize/2);//中心点（4,4）
...
//得到warp矩阵和搜索层级   根据warp矩阵的行列式（面积）确定搜索层级
                                              ...
                                              Matrix<2> &m2CamDerivs)
{
    ...
    //cout<<"mm2WarpInverse "<<mm2WarpInverse<<endl;
    //cout<<"dDet "<<dDet<<endl;
    mnSearchLevel = 0;
    ...
    while(dDet > 3 && mnSearchLevel < LEVELS-1)
    {
        mnSearchLevel++;
        dDet *= 0.25;
        //cout<<"mnSearchLevel "<<mnSearchLevel<<endl;
        //cout<<"dDet "<<dDet <<endl;
    };
...
void PatchFinder::MakeTemplateCoarseCont(MapPoint &p)
{
    ...
    // Still the same map point? Then compare warping matrix..  判断是否需要更新模板图
...
// coords.) Returns true on patch found.  在投影像素点的nRange范围内，与源图像块对比，查找匹配点
bool PatchFinder::FindPatchCoarse(ImageRef irPos, KeyFrame &kf, unsigned int nRange)
{
    ...
    for(; i<i_end; i++)          // For each corner ...  对每一个角点（irPos半径范围之内的角点）与
    {
        ...
        nSSD = ZMSSDAtPoint(L.im, *i);//对范围内的每一点，取8×8的块，和源8×8的块计算零均值SSD得分，得到匹配点irBest
...
bool PatchFinder::IterateSubPixToConvergence(KeyFrame &kf, int nMaxIts)
{
    ...
    int nIts;
    for(nIts = 0; nIts < nMaxIts && !bConverged; nIts++)//8次
    {
...
double PatchFinder::IterateSubPix(KeyFrame &kf)
{
    ...
    for(ir.y = 1; ir.y < mnPatchSize - 1; ir.y++)
    {
        ...
        for(ir.x = 1; ir.x < mnPatchSize - 1; ir.x++)
        {
            float fPixel =   // Calc target interpolated pixel  插值像素
