...
ATANCamera::ATANCamera(string sName)
{
    ...
    float mFOV = 31.33724;
    float fovRad = (mFOV/2.f)/180.0*3.14159;
    mHeightNearPlane = 2.f * 10.f * (float)std::tan(fovRad);
    mWidthNearPlane = ( mHeightNearPlane / 360.f ) * 640.f;
    gCoeffScreen2Img = 640 / mWidthNearPlane;
...
//    Vector<2> uv;
//    uv[0] = 320.0+vCam[0]*640.0/9.86;
//    uv[1] = 180.0-vCam[1]*640.0/9.86;
//    return uv;

    mvLastIm[0] = ( vCam[0]*10.f+mWidthNearPlane/2) * gCoeffScreen2Img;
    mvLastIm[1] = (-vCam[1]*10.f+mHeightNearPlane/2) * gCoeffScreen2Img;
...
//    mvLastIm = v2Im;
//    Vector<2> vv;
//    vv[0] = mvLastIm[0]/640.0*9.86-9.86/2.0;
//    vv[1] = -mvLastIm[1]/640.0*9.86-5.54/2.0;
//    return vv;

    mvLastCam[0] =  (v2Im[0]-320) / gCoeffScreen2Img / 10.f;
    mvLastCam[1] = -(v2Im[1]-180) / gCoeffScreen2Img / 10.f;
...
//    return m2Derivs;
      Matrix<2> m2Derivs;
      m2Derivs[0][0] = gCoeffScreen2Img*10.f;
      m2Derivs[1][0] = 0;
      m2Derivs[0][1] = 0;
      m2Derivs[1][1] = -gCoeffScreen2Img*10.f;
      return m2Derivs;
}
