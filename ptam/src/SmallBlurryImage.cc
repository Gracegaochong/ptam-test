...
pair<SE2<>,double> SmallBlurryImage::IteratePosRelToTarget(SmallBlurryImage &other, int nIterations)//上一帧 6
{//(*mpSBILastFrame, 6)上一帧模板图的梯度值 6次迭代
    SE2<> se2CtoC;
    SE2<> se2WfromC;
    ImageRef irCenter = mirSize / 2;
    se2WfromC.get_translation() = vec(irCenter);//定值
...
//获得当前帧图片的映射后图像
        ...
        do
        {
            ...
            Vector<2> v2SumGrad = 0.25 * (v2CurrentGrad  + other.mimImageJacs[ir]);//变形图片的梯度  上一帧图片的梯度
...
SE3<> SmallBlurryImage::SE3fromSE2(SE2<> se2, ATANCamera camera) 
{
    ...
    Vector<3> av3OrigPoints[2];   // 3D versions of these points.  像素值-归一化z=1
    ...
    for(int it = 0; it<3; it++)
    {
        ...
        for(int i=0; i<2; i++)
        {
            ...
            for(int m=0; m<3; m++)
            {
                const Vector<3> v3Motion = SO3<>::generator_field(m, v3Cam);//3*3
